...
public class comparison extends AppCompatActivity {
    EditText recentdistance , previousdistance , distancechange , recenttime , previoustime , timechange, previousspeed, recentspeed , speedchange , previouspace , recentpace , pacechange;
        ...
        if(!(db.getRoutes("").isEmpty())) {
            ...
            if (myroute != null) {
                if (myroute.size() >= 2) {
                    ...
                    pacechange.setText("" + ((myroute.get(myroute.size() - 1).getBestTime()) / ((60000) * myroute.get(myroute.size() - 1).getTotalDistance()) - ((myroute.get(myroute.size() - 2).getBestTime()) / ((60000) * myroute.get(myroute.size() - 2).getTotalDistance()))));
                }
